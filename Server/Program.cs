...
app.MapPost("/api/login", async (LoginRequest req) =>
{
    ...
    using var cmd = new MySqlCommand(ReadSql("rest_login.sql"), conn);
    ...
    if (await r.ReadAsync())
    {
        ...
        await r.CloseAsync();
        using var seen = new MySqlCommand(ReadSql("rest_update_last_seen.sql"), conn);
...
app.MapPost("/api/register", async (RegisterRequest req) =>
{
    using var conn = new MySqlConnection(connectionString);
    await conn.OpenAsync();
    using (var checkUser = new MySqlCommand(ReadSql("rest_check_username.sql"), conn))
    {
    ...
    using (var checkNick = new MySqlCommand(ReadSql("rest_check_nickname.sql"), conn))
    {
    ...
    using (var insert = new MySqlCommand(ReadSql("rest_register.sql"), conn))
    {
        ...
        using (var ensureNode = new MySqlCommand(ReadSql("rest_ensure_node.sql"), conn))
        {
        ...
        using var travel = new MySqlCommand(travelSql, conn);
...
app.MapGet("/api/inventory/{userId:int}", async (int userId) =>
{
    ...
    using (var cmd = new MySqlCommand(ReadSql("rest_inventory_load_items.sql"), conn))
    {
    ...
    using (var cmd = new MySqlCommand(ReadSql("rest_inventory_load_equipment.sql"), conn))
    {
        cmd.Parameters.AddWithValue("@id", userId);
        using var r = await cmd.ExecuteReaderAsync();
        while (await r.ReadAsync())
...
app.MapPost("/api/inventory/add", async (InventoryUpdate req) =>
{
    using var conn = new MySqlConnection(connectionString);
    await conn.OpenAsync();
    using var cmd = new MySqlCommand(ReadSql("rest_inventory_add_item.sql"), conn);
...
app.MapPost("/api/inventory/equip", async (EquipRequest req) =>
{
    ...
    {
        using var cmd = new MySqlCommand(ReadSql("rest_inventory_remove_equipment.sql"), conn);
        cmd.Parameters.AddWithValue("@id", req.UserId);
    ...
    {
        using var cmd = new MySqlCommand(ReadSql("rest_inventory_save_equipment.sql"), conn);
        cmd.Parameters.AddWithValue("@id", req.UserId);
        cmd.Parameters.AddWithValue("@character", req.CharacterName);
        cmd.Parameters.AddWithValue("@slot", req.Slot);
        cmd.Parameters.AddWithValue("@name", req.ItemName);
...
app.MapGet("/api/ability/{name}", async (string name) =>
{
    using var conn = new MySqlConnection(connectionString);
    await conn.OpenAsync();
    using var cmd = new MySqlCommand(ReadSql("rest_get_ability_by_name.sql"), conn);
