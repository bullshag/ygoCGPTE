...
- **Progress:** In progress, 90% (infoFrame auto-discovery, scene wiring, and combat shortcut live; metadata-driven population pending) — due 2025-09-29.
...
- **Implementation Notes (2025-09-24):**
  - `LocationActivitiesPanel` caches each button's `onClick` handler at `Awake`, uses `SelectLocationByName` helpers for direct triggers (Enter from `AreaTooltip`), and keeps the panel hidden until `Open()` activates the root.
  - When opened, the panel restores the previously active activity (or selects the first entry), forces only that content root active, and pushes focus to the active button for keyboard/gamepad navigation.
  - Escape key or the `Cancel` input action routes through `Close()`, which hides all content, unlocks the originating `AreaTooltip`, and clears the selected UI object so the tooltip can reopen without leaving the trigger.
  ...
  - Normalized auto-discovery name variants after merge cleanup so handcrafted backgrounds (Tavern, Shop, Temple, etc.) resolve before placeholders are generated.
- **Risks:**
...
- **Progress:** In progress, 35% (top-level button auto-wiring and candidate template preservation complete; detail overlay still pending) — due 2025-10-01.
...
- **Implementation Notes (2025-09-24):**
  - `TavernPanel` auto-resolves `searchPartyMembersBtn`, `hireMercBtn`, `questsBtn`, the Scroll View, and the embedded `slot` template under `tavernBackground`, disabling the template for reuse while leaving hire/work buttons marked as coming soon.
- **Risks:**
...
|---|---|---|---|---|---|
| Location Activities Panel Refresh | 90% (due 2025-09-29) | Codex | FEAT-WM-001; FEAT-UI-006; location metadata service (Owner: TBD, due 2025-09-27) | Reuses handcrafted `locationInfoWindow` layout with dynamic activity selection;<br>Auto-discovers `infoFrame` buttons, generates placeholders for unfinished facilities, and routes `Search for Enemies` into combat scene;<br>Supports 1080p/1440p layouts without blocking map input | InputAction focus clashes during sub-panel open (Owner: Codex, due 2025-09-30);<br>Metadata contract TBD may delay integration (Owner: TBD, due 2025-09-27);<br>Placeholder copy must transition to live data once metadata lands (Owner: Codex, due 2025-09-29) |
| Tavern Sub-Panel Integration | 35% (due 2025-10-01) | Codex | FEAT-UI-004; FEAT-UI-007; TavernManager API audit (Owner: TBD, due 2025-09-28) | Hire, Mercenary, Work actions expose stateful buttons;<br>Auto-wires `tavernBackground` buttons, Scroll View, and slot template to `TavernPanel` while keeping hire/work labeled as coming soon;<br>`tavern_subpanel_open` telemetry fires with location ID | Legacy edge cases from TavernForm undocumented (Owner: TBD, due 2025-09-29);<br>Responsive layout for small resolutions unverified (Owner: Codex, due 2025-10-03);<br>CharacterService refresh timing may double-trigger updates (Owner: Codex, due 2025-10-04) |
...
- 2025-09-24: Auto-wired `locationInfoWindow` to LocationActivitiesPanel, linked `tavernBackground` to TavernPanel, and documented the combat shortcut plus placeholder generation for upcoming facilities. — Codex
...
- 2025-09-24: Documented current LocationActivitiesPanel runtime behavior, focus handling, input shortcuts, and placeholder generation in Implementation Notes. — Codex
...
- 2025-09-24: Normalized LocationActivitiesPanel auto-discovery name variants to clear lingering merge artifacts and verify single authoritative scene wiring. — Codex
